...
* **Client Type for Writes**:
  ...
  - **Security Check**: The `userId` used in all Supabase updates/upserts (e.g. lines 42, 88, 102, 188, 229, 247) comes directly from `getAuthenticatedUser()` (session), **NOT** from the request body (`req.body`).
  - Thus, a user cannot update anyone else's profile by supplying a different ID, making the service role usage **acceptable for now**.
* **Tag**: 🟡 (RLS bypassed via service role client, but secure against profile spoofing because it enforces the session-derived user ID).
...
* **Tag**: 🟡 (Revoke session step (b) is missing).
