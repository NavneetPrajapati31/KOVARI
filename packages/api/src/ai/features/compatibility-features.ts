...
const dateOverlapScore = (
  ...
  const ratio = overlapDays / ((eA - sA) / (1000 * 60 * 60 * 24)); // Normalized against seeker's duration (solo)
  // Logic adjustment: in user_user, we use seek_duration = max(durA, durB) per original
  const durA = eA - sA;
  const durB = eB - sB;
  const denom = matchType === "user_group" ? durA : Math.max(durA, durB);
  
  if (denom <= 0) return 0 as NormalizedScore;

  const actualRatio = overlapMs / denom;
  if (actualRatio >= 0.8) return 1.0 as NormalizedScore;
  if (actualRatio >= 0.5) return 0.9 as NormalizedScore;
  if (actualRatio >= 0.3) return 0.8 as NormalizedScore;
  if (actualRatio >= 0.2) return 0.6 as NormalizedScore;
  if (actualRatio >= 0.1) return 0.3 as NormalizedScore;
