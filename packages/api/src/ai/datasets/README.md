...
```bash
python packages/api/src/ai/datasets/build_training_set.py match_events.jsonl
```

### Advanced Usage

```bash
python packages/api/src/ai/datasets/build_training_set.py \
  match_events.jsonl \
  --output-dir datasets \
  --train-ratio 0.8 \
  --min-samples 10
...
```bash
# Windows (recommended)
python -m pip install -r packages/api/src/ai/datasets/requirements.txt

# Or if pip is in PATH
pip install -r packages/api/src/ai/datasets/requirements.txt
```

## Data Collection (Pre-Launch)

**⚠️ IMPORTANT:** Before building datasets, read `DATA_COLLECTION_GUIDE.md` (if available in original source)
...
node packages/api/src/ai/datasets/seed-training-data.js
...
.\packages\api\src\ai\datasets\extract-events.ps1 app.log match_events.jsonl
