...
export async function GET(req: Request) {
  ...
  try {
    ...
    console.log(
      "[GET /api/admin/sessions/search] Found",
      allKeys.length,
      "total session keys"
    );
    ...
    for (
      let i = 0;
      i < allKeys.length && matchingSessions.length < limit;
      i++
    ) {
      const key = allKeys[i];
      ...
      try {
        ...
        const userId =
          ...
          typeof staticAttrs.clerkUserId === "string"
            ? staticAttrs.clerkUserId
            : null) ??
        ...
        console.error(
          `[GET /api/admin/sessions/search] Error processing key ${key}:`,
          keyErr
        );
        continue;
      }
    ...
    Sentry.captureException(error, {
      tags: {
        scope: "admin-api",
        route: "GET /api/admin/sessions/search",
      },
    });
    throw error;
  }
