...
const calculateDynamicWeights = (
  ...
  if (totalBoost > 0) {
    ...
    const remainingWeight =
      1 -
      Object.entries(weights).reduce((sum, [key, value]) => {
    ...
    if (remainingWeight > 0) {
      ...
      Object.keys(weights).forEach((key) => {
        if (
          !coreFilters.includes(key) &&
          !filterBoost[key as keyof FilterBoost]
        ) {
...
const calculateReligionScore = (r1?: string, r2?: string): number => {
  ...
  if (
    neutralReligions.includes(r1.toLowerCase()) ||
    neutralReligions.includes(r2.toLowerCase())
  )
    return 0.5;
  return 0;
};

const calculateLocationOriginScore = (
...
const calculateLifestyleScore = (
  attrs1: StaticAttributes,
  attrs2: StaticAttributes
): number => {
...
export const calculateFinalCompatibilityScore = (
  ...
  const baseWeights = {
    ...
    budget: 0.2, // Third priority - spending capacity
    interests: 0.1, // Fourth priority - common interests
    age: 0.1, // Fifth priority - age compatibility
  ...
  const weights = filterBoost
    ? calculateDynamicWeights(baseWeights, filterBoost)
    : baseWeights;

  const userAttrs =
    userSession.static_attributes || ({} as Partial<StaticAttributes>);
  const matchAttrs =
    matchSession.static_attributes || ({} as Partial<StaticAttributes>);
  ...
  const scores = {
    destinationScore: calculateDestinationScore(
      userSession.destination,
      matchSession.destination
    ),
    dateOverlapScore: calculateDateOverlapScore(
      userSession.startDate,
      userSession.endDate,
      matchSession.startDate,
      matchSession.endDate
    ),
    personalityScore: getPersonalityCompatibility(
      userAttrs?.personality || "unknown",
      matchAttrs?.personality || "unknown"
    ),
    interestScore: calculateJaccardSimilarity(
      userAttrs?.interests || [],
      matchAttrs?.interests || []
    ),
    budgetScore: calculateBudgetScore(userSession.budget, matchSession.budget),
    religionScore: calculateReligionScore(
      userAttrs?.religion || "unknown",
      matchAttrs?.religion || "unknown"
    ),
    locationOriginScore: calculateLocationOriginScore(
      userAttrs?.location || { lat: 0, lon: 0 },
      matchAttrs?.location || { lat: 0, lon: 0 }
    ),
...
export const isCompatibleMatch = (
  ...
  const distance = getHaversineDistance(
    ...
    matchSession.destination.lat,
    matchSession.destination.lon
  );
...
export const createFilterBoost = (
  filters: Partial<FilterBoost>
): FilterBoost => {
