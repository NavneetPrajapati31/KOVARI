...
export async function POST(req: NextRequest) {
  try {
    ...
    const [
      ...
      // 1. Skips: Exclude if I skipped the group (unless in demo mode)
      shouldExcludeSkips
        ? supabase
            .from("match_skips")
        ...
        : Promise.resolve({ data: [], error: null }), // Return empty in demo mode 
    ...
    // Add Skips (only if not in demo mode)
    if (shouldExcludeSkips) {
      skipsResult.data?.forEach(s => excludedGroupIds.add(s.skipped_user_id));
      console.log(`🎬 Demo mode: Skipped groups will reappear in results`);
    } else {
      console.log(`🎬 Demo mode: Allowing skipped groups to appear (${skipsResult.data?.length || 0} skips ignored)`);
    }
    // Add Reports
