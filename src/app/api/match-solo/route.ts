...
export async function GET(request: NextRequest) {
  try {
    ...
    // Use the isDemoMode variable already defined above for demo mode check
    const allSessions: SoloSession[] = validSessionsJSON
      ...
      .filter((session: SoloSession) => {
        ...
        // Exclude mock/test users unless demo mode is enabled
        // Mock users have patterns like: user_mumbai_1, user_test_1 (descriptive names)
        // Real Clerk IDs are long random alphanumeric strings
        const isMockUserPattern = session.userId && /^user_[a-z][a-z0-9_]*$/.test(session.userId);
        if (isMockUserPattern) {
          if (isDemoMode) {
            console.log(`🎬 Demo mode: Allowing mock user: ${session.userId}`);
          } else {
            console.log(`🚫 Filtering out mock/test user: ${session.userId}`);
            return false;
          }
        }
        return true;
