...
export function GroupCard({ group, className }: GroupCardProps) {
  ...
  return (
    <Link href={`/groups/${group.id}/home`} className="block">
      <Card
        className={cn(
          "flex flex-row items-center gap-x-2 px-4 bg-card text-foreground shadow-none border-none cursor-pointer transition-colors hover:bg-muted/50",
          className
        )}
      >
        {/* Avatar */}
